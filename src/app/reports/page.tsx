...
export default async function ReportsPage() {
  ...
  const { data: monthlySales } = await supabase
    .from("sales")
    .select(
      "id, product_name, brand_name, quantity_sold, selling_price, cost_price_at_sale, profit, sold_at, stock_item_id",
    )
  ...
  const { data: stockItems } = await supabase
    .from("stock_items")
    .select(
      "id, imei, variant, cost_price, status, condition_notes, purchased_at, sold_at, product_id, sale_id",
    )
  ...
  const grossRevenue = (monthlySales ?? []).reduce(
    (sum, s) => sum + Number(s.selling_price),
    0,
  );
  const grossProfit = (monthlySales ?? []).reduce(
  ...
  const lastMonthProfit = (lastMonthSales ?? []).reduce(
    (sum, s) => sum + Number(s.profit),
    0,
  );
